...
import typing
import lmdb
from training.data import PklDataset
...
class Resize(object):
    ...
    # def _call_ar_perserve_short_axis_(self, img: typing.Union[np.ndarray, Image.Image]):
    #     if isinstance(img, np.ndarray):
    #         interpolation = self.interpolation if self.interpolation is not None else cv2.INTER_LINEAR
    #         old_size = list(reversed(img.shape[:2]))  # old_size is in (height, width) format
    #         depth = img.shape[2] if len(img.shape) > 1 else 1
...
def parse_flicker(flicker_text):
    with open(flicker_text, 'r') as fp:
        lines = fp.readlines()
    img_and_captions = []
    for line_num, line in enumerate(lines):
        img_, caption = line.split('\t')
        img, ann = img_.split('#')
        img_and_captions.append((img.strip(), caption.strip(), line_num))

    return img_and_captions


def cap2token(caption, tokenizer, max_length=None, pad_to_max_length=False):
    import transformers
    if int(transformers.__version__.split('.')[0]) > 2:
        if pad_to_max_length is True:
            pad_to_max_length = 'max_length'
        token = tokenizer.encode(caption, add_special_tokens=True, max_length=max_length,
                                 padding=pad_to_max_length, truncation=False)
    else:
        token = tokenizer.encode(caption, add_special_tokens=True, max_length=max_length,
                                 pad_to_max_length=pad_to_max_length, truncation=False)
    token_mask = tokenizer.get_special_tokens_mask(token, already_has_special_tokens=True)
    token = torch.tensor(token)
    token_mask = [not (t == 0 or m > 0) for m, t in zip(token_mask, token)]
...
class LmdbDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.timer:
            ...
            print(self.timer)
        caption = self.annotations[ann_id]['queries'][q_id]

        caption_words, caption_tokens, cap_tokens_mask = self.sen2token(caption)

        if self.timer:
            self.timer.toc('Embed Caption')
            print(self.timer)
        annid_qid = f'{ann_id},{q_id}'
        _, phrases_tok, phrases_tok_mask = self.get_tokens(ann_id, q_id)
        caption_words = np.array(caption_words).astype(np.string_)
        return (image, mask, caption_tokens, cap_tokens_mask, caption_words, annid_qid, phrases_tok,
                phrases_tok_mask, orig_image_size)

    def sen2token(self, caption):
        if not isinstance(caption, list):
            caption = caption.replace('’', "'")
            caption = caption.replace('.', '').replace(',', '').replace('!', '').replace('?', '').lower().strip()
            words = caption.split()
        else:
            words = [w.replace('’', "'") for w in caption]
        return words
    ...
    def get_tokens(self, ann_id, q_id):
        ...
        for phrase in self.annotations[ann_id]['annotations']:
            ...
            if 'query' in phrase.keys():
                ...
                if src_sen == self.annotations[ann_id]['queries'][q_id]:
                    q = phrase['query']
                else:
                    q = None
            else:
            ...
            phrases.append(np.array(words).astype(np.string_))
        ...
        return np.array(phrases), phrases_tokens, phrases_tok_mask
    ...
    def collate_fn(cls, tokenizer: transforms, debug_timing=False):
        ...
        def fun(data):
            ...
            img, mask, full_cap_tokens, full_cap_masks, cap_words, ann_id, queries_tok, q_tok_emb_mask, orig_image_size = zip(*data)
            ...
            for c_ind, (cap_emb, cap_mask) in enumerate(zip(queries_tok, q_tok_emb_mask)):
                ...
                for q_ind in range(len(cap_emb), max_phrases):
                    caps[q_ind].append(torch.tensor([101, 102], dtype=torch.int64))
                    caps_masks[q_ind].append(torch.tensor([1, 1], dtype=torch.int64))
            ...
            return cls.Batch(img, mask, torch.stack(all_emb), torch.stack(all_masks),
                             np.array(ann_id).astype(np.string_), torch.stack(orig_image_size),
                             full_cap_tokens, full_cap_masks)

        return fun


class LmdbPairDatasetCLIP(LmdbDataset):
    def __init__(self, dataset_path, split, resize_transform=None, transform=None, debug_timing=False, max_len=None,
                 clip_tokenizer=None, clip_preprocess=None, eval_data_split='coco1k'):
        super(LmdbPairDatasetCLIP, self).__init__(dataset_path[0], split, resize_transform=resize_transform,
                                                  transform=transform, debug_timing=debug_timing, max_len=max_len)
        self.clip_tokenizer = clip_tokenizer
        self.clip_preprocess = clip_preprocess
        self.coco_class_list = coco_class_list
        img_ids = []
        if 'coco' in eval_data_split:
            ids_file_name = f'mm_retrieval/{eval_data_split}_ids.txt'
            with open(ids_file_name, 'rb') as f:
                for line in f:
                    img_ids.append(str(int(line)))
            self.img_ids = list(set(img_ids))
        elif 'flickr' in eval_data_split:
            for (ann_id, q_id) in self.ids:
                if ann_id not in img_ids:
                    img_ids.append(ann_id)
    ...
    def __getitem__(self, index):
        """Returns one data pair (image and caption)."""
        # ann_id, q_id = self.ids[index]
        ann_id = self.img_ids[index]
        if len(self.annotations[ann_id]['queries']) == 0:
            raise ValueError
        if os.path.exists(os.path.join(self.raw_im_path, f'{ann_id}.jpg')):
            image = Image.open(os.path.join(self.raw_im_path, f'{ann_id}.jpg'))
            if image is None:
                print(f'Could not read image {os.path.join(self.raw_im_path, f"{ann_id}.jpg")}')
                imgbin = self.txn.get(ann_id.encode('utf-8'))
                if imgbin is not None:
        ...
        # orig_image_size = torch.tensor(image.shape[1:]).half()
        # caption = self.annotations[ann_id]['queries'][q_id]
        captions = self.annotations[ann_id]['queries']
        # caption_words = self.sen2token(caption)
        # text_inputs = torch.cat([self.tokenizer(word) for word in caption_words]).cuda()
        text_inputs = self.clip_tokenizer(captions)
        # text_inputs_test = [self.clip_tokenizer(caption) for caption in captions]
        # annid_qid = f'{ann_id},{q_id}'
        # caption_words = np.array(caption_words).astype(np.string_)
        img_label = torch.tensor(index)
        txt_labels = torch.ones(len(captions), dtype=int) * index
        return (image, text_inputs, img_label, txt_labels)

    @classmethod
    def collate_fn(self):
        # noinspection PyUnreachableCode
        def fun(data):
            img, text_inputs, img_labels, txt_labels = zip(*data)
            img = torch.stack(img, 0)
            # max_len = 0
            # for text_in in text_inputs:
            #     max_len = max(max_len, text_in.shape[0])
            text_inputs = torch.cat(text_inputs, 0).squeeze()
            img_labels = torch.stack(img_labels, 0).squeeze()
            txt_labels = torch.cat(txt_labels, 0).squeeze()
            return img, text_inputs, img_labels, txt_labels
        return fun

    class Batch(BaseBatch):
        def __init__(self, image, text_inputs, img_labels, txt_labels):
            """
            :param image:
            :type  image: torch.Tensor
...
class LmdbPairDatasetCLIPVal(LmdbPairDatasetCLIP):
    def __getitem__(self, index):
        """Returns one data pair (image and caption)."""
        ann_id, q_id = self.ids[index]
        gt_labels = [anno['category'] for anno in self.annotations[ann_id]['annotations']]
        while not gt_labels:
            ann_id, q_id = self.ids[random.randrange(0, self.__len__())]
            gt_labels = [anno['category'] for anno in self.annotations[str(int(ann_id))]['annotations']]
        if len(self.annotations[ann_id]['queries']) == 0:
            raise ValueError
        if os.path.exists(os.path.join(self.raw_im_path, f'{ann_id}.jpg')):
            image = Image.open(os.path.join(self.raw_im_path, f'{ann_id}.jpg'))
        ...
        image = self.clip_preprocess(image)
        gt_labels_indices = [self.coco_class_list.index(gt_label) for gt_label in gt_labels]
        gt_labels_1_hot = self.labels_list_to_1hot(gt_labels_indices)
        # orig_image_size = torch.tensor(image.shape[1:]).half()
        # caption = self.annotations[ann_id]['queries'][q_id]
        # caption_words = self.sen2token(caption)
        # text_inputs = torch.cat([self.tokenizer(word) for word in caption_words]).cuda()
        # text_inputs = self.clip_tokenizer(caption)
        # annid_qid = f'{ann_id},{q_id}'
        # caption_words = np.array(caption_words).astype(np.string_)
        return (image, gt_labels_1_hot)
    ...
    def collate_fn(self):
        ...
        def fun(data):
            img, ann_id = zip(*data)
            img = torch.stack(img, 0)
            ann_id = torch.stack(ann_id, 0)
            return img, ann_id
        return fun

    class Batch(BaseBatch):
        def __init__(self, image, ann_id):
            """
            :param image:
            :type  image: torch.Tensor
    ...
    def labels_list_to_1hot(self, labels_list):
        """Convert a list of indices to 1hot representation."""
        labels_1hot = torch.zeros(len(self.coco_class_list))
        labels_1hot[labels_list] = 1
        return labels_1hot

imagenet_id_to_class_name = {0: 'tench, Tinca tinca',
